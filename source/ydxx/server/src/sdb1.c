...
bool parse_sdb_general(int ver, Shmem *src, Gene *gene)
{
	...
	shmem_read_int(src, &gene->level);
	shmem_read_int(src, &gene->level_percent);
...
void create_sdb_wubao(int version, Wubao *w, dstring *dst)
{
	...
	dstring_write_int(dst, w->last_login_time);
	dstring_write_int(dst, w->been_plunder_num);
	dstring_write_int(dst, w->use_gx_trea_num);
	dstring_write_int(dst, w->war_sleep_end_time);
	dstring_write_int(dst, w->train_sleep_end_time);
	dstring_write_int(dst, w->box_level);
	dstring_write_int(dst, w->rank);
	dstring_write_int(dst, w->gx);
	dstring_write_int(dst, w->jl);
	dstring_write_int(dst, w->fresh_step);
...
bool parse_sdb_wubao(int ver, Shmem *src, Wubao *w)
{
	...
	shmem_read_int(src, &w->last_login_time);
	shmem_read_int(src, &w->been_plunder_num);
	shmem_read_int(src, &w->use_gx_trea_num);
	shmem_read_int(src, &w->war_sleep_end_time);
	shmem_read_int(src, &w->train_sleep_end_time);
	shmem_read_int(src, &w->box_level);
	shmem_read_int(src, &w->rank);
	shmem_read_int(src, &w->gx);
	shmem_read_int(src, &w->jl);
	shmem_read_int(src, &w->fresh_step);
