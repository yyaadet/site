package map
{
	...
	public class AStar
	{
		...
		//地形类型
		//0:无，1：城市，2：关隘，3：大路，4：小路，5：山路，6:河流，7：江海
		//8：平原，9：草原，10：森林，11：沙漠，12：丘陵，13：山地，14：高地，15：高原
		private var _landType:Array = null;
		
		private var _costType:Array = null;
		 ...
		 */		
		public function AStar(p_maxTry : int = 0)
		{
		 ...
		 */		
		public function find(p_startX : int, p_startY : int, p_endX : int, p_endY : int, p_navy:Boolean) : Array
		{
			this.initLists();
			this.m_openCount = 0;
			this.m_openId = -1;
			...
			var type:int = landType[p_endX][p_endY];
			if (isNavy)
			{
			}
			else
			...
			while (this.m_openCount > 0)
			{
				...
				{
					MapManager.mapMgr.showAStarFault();
					this.destroyLists();
					return null;
				}
				...
				for each (var note : Array in aroundNotes)
				{
					//计算F和G值
					//G
					cost = this.m_movementCostList[currId] + getCost(currNoteX, currNoteY) * ((note[0] == currNoteX || note[1] == currNoteY) ? COST_STRAIGHT : COST_DIAGONAL);
					//F = G + H
		...
		private function getArounds(p_x : int, p_y : int) : Array
		{
			...
			checkX = p_x + 1;
			checkY = p_y;
			var canRight : Boolean = canPass(p_x, p_y, checkX, checkY);
			...
			checkX = p_x;
			checkY = p_y + 1;
			var canDown : Boolean = canPass(p_x, p_y, checkX, checkY);
			...
			checkX = p_x - 1;
			checkY = p_y;
			var canLeft : Boolean = canPass(p_x, p_y, checkX, checkY);
			...
			checkX = p_x;
			checkY = p_y - 1;
			var canUp : Boolean = canPass(p_x, p_y, checkX, checkY);
			...
			checkX = p_x + 1;
			checkY = p_y + 1;
			canDiagonal = canPass(p_x, p_y, checkX, checkY);
		 ...
		 * 
		 */		
		private function canPass(p_startX : int, p_startY : int, p_endX : int, p_endY : int):Boolean
		{
			var ret:Boolean = true;
			if (p_endX >= MapUtil.mapRow * MapUtil.sliceWidth / MapUtil.mapGridW ||
			    p_endY >= MapUtil.mapCol * MapUtil.sliceHeight / MapUtil.mapGridH)
			{ 
				ret = false;   
			    return ret;
			}
		 ...
		 * 
		 */		
		private function getCost(p_x:int, p_y:int):int
		{
			var ret:int = 0;
			
			ret = costType[p_x][p_y];
		...
		public function get landType():Array
		{
			return _landType;
		}
		
		public function set landType(param:Array):void
		{
			_landType = param;
		}
		
		public function get costType():Array
		{
			return _costType;
		}
		
		public function set costType(param:Array):void
		{
			_costType = param;
		}
